...
--
-- Supabase's auth.users has several NOT-NULL text columns (confirmation_token,
-- recovery_token, etc.) with default '' — explicit empty strings here keep us
-- compatible across GoTrue versions.

-- ============================================================================
-- Demo auth users (instructors) — demo-only; passwords are unusable hashes.
-- ============================================================================
...
values
  ('00000000-0000-0000-0000-000000000000',
   ...
   'priya@nuvexa.demo',
   crypt('nuvexa-demo-instructor', gen_salt('bf')),
   now(),
...
-- Sanity check: if auth.users inserts didn't happen, raise a clear error.
do $$
begin
  if (select count(*) from auth.users where id in (
    ...
    raise exception 'Demo auth.users were not created — cannot seed profiles. Check Supabase auth schema version.';
  end if;
end $$;
...
-- ============================================================================
-- Lessons (a few per course, one trial each)
-- ============================================================================
